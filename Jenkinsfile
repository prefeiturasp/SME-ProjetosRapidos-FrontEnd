pipeline {
    environment {
      ...
      namespace = "${env.branchname == 'develop' ? 'projetosrapido-dev' : env.branchname == 'homolog' ? 'projetosrapido-hom' : env.branchname == 'homolog-r2' ? 'projetosrapido-hom2' : 'sme-projetosrapidos' }"
    }
        ...
        stage('AnaliseCodigo') {
          ...
          steps {
              withSonarQubeEnv('sonarqube-local'){
                ...
                -Dsonar.projectKey=SME-ProjetosRapidos-FrontEnd \
                -Dsonar.sources=.'
            }
          }
        }        
            ...
            steps {
                script{                        
                    withCredentials([file(credentialsId: "${kubeconfig}", variable: 'config')]){
                        ...
                        sh 'kubectl rollout restart deployment/projetosrapidos-front -n ${namespace}'                            
                    }                    
                }
            }           
        } 
